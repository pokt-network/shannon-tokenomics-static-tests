...
def get_relay_cost_usd(compute_cost, compute_unit_fee):
    return compute_cost*compute_unit_fee
...
def get_usd_target_mint(compute_unit_fee, deflation_threshold, daily_CUs, daily_relays):
    '''
    ...
    return deflation_threshold*avg_cu_per_relay*compute_unit_fee
...
def get_Validators_Supplier_boost(service_CUs, service_compute_cost, service_CUTTM, mint_share):
    '''
    Computes how much of the total minted rewards (in excess) belong to the 
    Validator.
    '''
    ...
    *** I dont really understand this ***
    ...
    if daily_relays < floor_relays:
        ...
        if daily_relays > cutoff_point:
            ...
            return prop_of_CUF - (prop_of_CUF-prop_ceiling)*(daily_relays-floor_relays)/(cutoff_point-floor_relays)
        
def get_mint_burn_reward_Supplier(service_CUs, POKT_value, compute_unit_fee, mint_share):
    '''
    *** I dont really understand this ***

    Calculates the total POKT that is given to sources from the burning=minting mechanism
    '''
    computed_usd = service_CUs*compute_unit_fee # total USD computed
...
# def get_boost_reward_Supplier(data_df, POKT_value, compute_unit_fee, mint_share):
#     computed_usd = data_df['Total CUs']*compute_unit_fee # total USD computed
#     computed_usd = mint_share['Supplier']*computed_usd # Share of computed usd value
#     return computed_usd/POKT_value # Share of computed POKT value

def get_sources_prop_of_their_burn_from_CUF(relays, gfpr_rar_ceiling_relays, gfpr_rar_prop_ceiling, gfpr_rar_floor_relays, gfpr_rar_prop_floor):
    '''
    *** I dont really understand this ***
    ...
    if relays >= gfpr_rar_ceiling_relays:
        ...
        if relays <= gfpr_rar_floor_relays:
            ...
            # A linear equation
            return gfpr_rar_prop_ceiling + (gfpr_rar_prop_floor-gfpr_rar_prop_ceiling) * (relays-gfpr_rar_ceiling_relays) / (gfpr_rar_floor_relays-gfpr_rar_ceiling_relays)
        
def get_uncapped_reward_sources_boost(service_CUs, prop_of_their_burn_from_CUF,compute_unit_fee, prop_gfpr_Suppliers, POKT_value):
    '''
    *** I don't really understand this ***
    ...
    '''
    total_usd = service_CUs*compute_unit_fee # total USD from CUs
    proportion_usd = total_usd*prop_of_their_burn_from_CUF
    proportion_usd = proportion_usd*prop_gfpr_Suppliers # Additional proportion? this further reduce the income of suppliers
...
def process(data_df: pd.DataFrame, network_macro: dict, params: dict) -> Tuple[pd.DataFrame, dict]:
    ...
    # Gateway fee per CU 
    GFPCU = network_macro['compute_unit_fee']/network_macro['POKT_value']

    # Calculate total CUs for each service
    data_df['Total CUs'] = get_CUs(data_df['Compute Cost'], data_df['Relays'])

    # Calculate gateway cost per relay for each service
    data_df['Gw Cost per Relay USD'] = get_relay_cost_usd(data_df['Compute Cost'],network_macro['compute_unit_fee'])

    # Total burnt for each service
    data_df['Total burn POKT'] = get_burned_POKT(data_df['Compute Cost'], data_df['Relays'],GFPCU)
    ...
    # Target mint in USD
    usd_target_mint = get_usd_target_mint(network_macro['compute_unit_fee'], params['deflation_threshold'], daily_CUs, network_macro['daily relays'])

    # Calculate excess POKT to mint (excess from burn)
    mint_per_day = get_mint_per_day(params['max_mint_per_day'], usd_target_mint, network_macro['POKT_value'], total_burned)

    # Calculate the CUTTM for each service
    data_df['Base CUTTM'] = get_base_CUTTM(data_df['Total CUs'], data_df['Compute Cost'], daily_CUs, mint_per_day)


    # Get the excess POKT minted to each actor
    data_df['Suppliers'] = get_Suppliers_Supplier_boost(data_df['Total CUs'], data_df['Compute Cost'], data_df['Base CUTTM'], network_macro['mint_share'])
    data_df['DAO'] = get_DAO_Supplier_boost(data_df['Total CUs'], data_df['Compute Cost'], data_df['Base CUTTM'], network_macro['mint_share'])
    data_df['Validators'] = get_Validators_Supplier_boost(data_df['Total CUs'], data_df['Compute Cost'], data_df['Base CUTTM'], network_macro['mint_share'])
    ...
    prop_gfpr_Suppliers = get_prop_of_gfpr_Supplier_boost(network_macro['daily relays'], params['prop_of_CUF'], params['floor_relays'], params['cutoff_point'], params['prop_ceiling'])
    # Get all sources rewards form burning/minting parity
    data_df['Mint Burn Reward Supplier'] = get_mint_burn_reward_Supplier(data_df['Total CUs'], network_macro['POKT_value'], network_macro['compute_unit_fee'], network_macro['mint_share'])
    # Get proportion ofrewards from total taht comes from burning/minting
    data_df['Sources proportion of their burn from CUF'] = data_df['Relays'].apply(lambda x: get_sources_prop_of_their_burn_from_CUF(x, params['gfpr_rar_ceiling_relays'], params['gfpr_rar_prop_ceiling'], params['gfpr_rar_floor_relays'], params['gfpr_rar_prop_floor']))
    # Calcualte the (uncapped) boost to mint for each Supplier
    data_df['uncapped_reward_sources_boost'] = get_uncapped_reward_sources_boost(data_df['Total CUs'], data_df['Sources proportion of their burn from CUF'], network_macro['compute_unit_fee'], prop_gfpr_Suppliers, network_macro['POKT_value'])
    # Get the capped rewards
    data_df['reward_sources_boost'] = get_reward_sources_boost(data_df['uncapped_reward_sources_boost'], data_df['uncapped_reward_sources_boost'].sum(), params['gfpr_rar_max_supplier_boost'])
    ...
    for idx, row in data_df.iterrows():
        ...
        if row['Relays'] > 0:
            ...
            result_dict['Chains'][row['Chain']]['burn_total'] = GFPCU*row['Relays']*row['Compute Cost']
            # How much we minted here due to work
            result_dict['Chains'][row['Chain']]['mint_base'] = row['Suppliers'] + row['DAO'] + row['Validators'] + result_dict['Chains'][row['Chain']]['burn_total']
            ...
            result_dict['Total_Mint_DAO'] += network_macro["mint_share"]["DAO"]*result_dict['Chains'][row['Chain']]['burn_total'] + row['DAO']
            result_dict['Total_Mint_Validator'] += network_macro["mint_share"]["Validator"]*result_dict['Chains'][row['Chain']]['burn_total'] + row['Validators']
            result_dict['Total_Mint_Supplier'] += network_macro["mint_share"]["Supplier"]*result_dict['Chains'][row['Chain']]['burn_total'] + row['Suppliers']
            result_dict['Total_Mint_Source'] += network_macro["mint_share"]["Source"]*result_dict['Chains'][row['Chain']]['burn_total'] + row['reward_sources_boost']
